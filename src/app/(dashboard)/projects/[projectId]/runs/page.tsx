...
export default function ProjectRunsPage() {
  ...
  function getStatusBadge(status: string) {
    ...
    return (
      <Badge variant="outline" className={cn("font-semibold text-[10px] tracking-widest uppercase px-3 py-1 rounded-full shadow-none", variants[status] || "")}>
        {status.toLowerCase()}
      </Badge>
    );
  }
      ...
      >
        ...
        <div className="flex items-center justify-between mb-8">
          <div className="flex items-center gap-4">
            ...
            <h1 className="text-4xl font-display font-normal tracking-tight">Test Runs</h1>
          </div>
        ...
        <div className="grid gap-4 grid-cols-4 mb-8">
          {[
            ...
            { label: "Running", value: runs.filter((r) => r.status === "RUNNING").length, color: "text-primary" },
          ].map((stat, i) => (
            <motion.div
              key={stat.label}
              initial={{ opacity: 0, y: 20 }}
            ...
            >
              <Card className="rounded-[2rem] border-border/40 shadow-glass backdrop-blur-xl bg-card/40">
                <CardContent className="p-5">
                  <p className="text-xs font-bold tracking-widest uppercase text-muted-foreground mb-2">{stat.label}</p>
                  <p className={cn("text-4xl font-display font-normal tracking-tight", stat.color)}>
                    {stat.value}
                  </p>
                </CardContent>
              </Card>
        ...
        {filteredRuns.length === 0 ? (
          <Card className="rounded-[2.5rem] border-border/40 shadow-glass backdrop-blur-xl bg-card/40">
            <CardContent className="flex flex-col items-center justify-center py-20">
              ...
              <p className="text-xl font-semibold mb-2">
                {statusFilter === "all" ? "No runs yet" : `No ${statusFilter.toLowerCase()} runs`}
              </p>
          ...
          <div className="space-y-4">
            {filteredRuns.map((run, i) => (
              ...
              >
                <Link href={`/projects/${projectId}/tests/${run.test.id}/runs/${run.id}`}>
                  <Card className="group rounded-[2rem] border-border/40 shadow-glass backdrop-blur-xl bg-card/40 hover:bg-card/60 hover:border-primary/20 hover:shadow-xl hover:-translate-y-0.5 transition-all duration-500 cursor-pointer">
                    <CardContent className="p-5">
                      <div className="flex items-center gap-4">
                        ...
                        <div className="flex-1 min-w-0">
                          <p className="font-semibold text-lg">{run.test.name}</p>
